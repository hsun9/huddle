...
{   
    ref <- unique(obj$ref)
    # min TSS
    min_tss <- round(quantile(seu$TSS.enrichment, probs = .1, na.rm=T))
    if (min_tss < 2){ min_tss = 2}

    # max mt
    max_mt <- round(quantile(seu$percent.mt, probs = .96, na.rm=T))
    if (max_mt > 10){ max_mt = 10}

    # max black list
    max_bl <- round(quantile(seu$blacklist_ratio, probs = .96, na.rm=T), 2)
    if (max_bl > 0.05){ max_bl = 0.05}

    # max nCount_RNA ~50000
    max_ncount_rna <- round(quantile(seu$nCount_RNA, probs = .996, na.rm=T)/10000) * 10000 
