...
FilterDoublet <- function(obj, outdir)
{   
    ...
    pdf(paste0(outdir, '/doublet.', sample_name, '.umap.rna.pdf'), width = 6, height = 5, useDingbats=FALSE)
    p <- DimPlot(obj, group.by = 'doubletfinder')
    print(p)
    dev.off()

    # output for metadata with doublet results
    write.table(obj@meta.data, file = paste0(outdir, '/doublet.', sample_name, '.metadata.xls'), sep = "\t", quote=F, col.names = NA)
