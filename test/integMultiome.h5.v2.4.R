...
for (name in folder_names){
    ...
    fragment <- paste0(rdir, '/', name, '/outs/atac_fragment.tsv.gz')
    ...
    # make data list
    multiome_list <- append(multiome_list, seurat_obj)
}
...
snrna_list <- base::lapply(X = multiome_filtered_list, FUN = function(x) { x <- SCTransform(x, vst.flavor = "v2", verbose = FALSE) })
