...
def CellrangerARC_Count_Multiple(fbsub, fscript, fconfig, model, ftable, outdir):
    info = pd.read_csv(ftable, sep='\t', names=['ref', 'name', 'ncell', 'library'])
    ...
    for index, row in info.iterrows():
        ...
        cmd = f'sh {fbsub} 64 16 arc_{name} bash {fscript} -C {fconfig} -R {model} -n {ncell} -G {ref} -L {library} -N {name} -O {outdir}'
        os.system(cmd)
